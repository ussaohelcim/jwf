...
const jwCL = {
	checkCollisionRecs(rect1:IRect,rect2:IRect){
		return (rect1.x < (rect2.x + rect2.w) && (rect1.x + rect1.w) > rect2.x) &&
		(rect1.y < (rect2.y + rect2.h) && (rect1.y + rect1.h) > rect2.y)
	},
	checkCollisionCircles(circle1:ICircle,circle2:ICircle){
	...
	checkCollisionCircleRec(center:ICircle,rec:IRect){
		let collision = false;
    ...
    if (dx > (rec.w/2.0 + center.r)) { return false; }
    if (dy > (rec.h/2.0 + center.r)) { return false; }

    if (dx <= (rec.w/2.0)) { return true; }
    if (dy <= (rec.h/2.0)) { return true; }
	...
	checkCollisionPointRec(point:IVec2,rec:IRect){
		return (point.x >= rec.x) && (point.x <= (rec.x + rec.w)) && (point.y >= rec.y) && (point.y <= (rec.y + rec.h))
	},
	checkCollisionPointCircle(point:IVec2,circle:ICircle){
		return this.checkCollisionCircles({x:point.x,y:point.y,r:1},circle)
	},
	checkCollisionPointTriangle(point:IVec2,triangle:ITriangle){
		let p1 = triangle.p1
		let p2 = triangle.p2
		let p3 = triangle.p3
